...
function initMapTips(data,mapType) {
    if($(".echartIconBox").find("li").length<=0){
        ...
        for(var i=0;i<data.length;i+=2){
            liString += "<li><div class='layui-input-block layui-form layui-form-item' data-icon="+data[i].icon+"><input type='checkbox' lay-filter='choose' class='mapTipsCheck'  name='"+data[i].id+"'><i class='iconfont' style='color:"+data[i].color+"'>"+data[i].icon+"</i>"+data[i].name+"</div>";
                ...
                !function(i){
                    ...
                    marker11.on('click', function(e) {
                        ...
                        layer.open({
                            ...
                            content: '<div style="padding:10px;"><span style="display:flex;height:25px;line-height:25px;border-bottom:1px solid #ccc">'+positions[i].name+'</span><img src="'+positions[i].imagesTitle+'" style="width:100px;height:100px;margin-top:10px;"><a id="mapImage_'+positions[i].no+'" style="display: block" onclick="showPhoto(\''+positions[i].images+'\')">查看相册</a><a id="mapImage_'+positions[i].no+'" style="display: block">查看简介</a><a href="http://www.mltc.cn/" target="new_window" id="mapImage_'+positions[i].no+'" style="display: block">前往官网</a></div>'//捕获的元素，注意：最好该指定的元素要存放在body最外层，否则可能被其它的相对元素所影响
                        });
                    });
                }(i);
            }
